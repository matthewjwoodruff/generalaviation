#########################################################
# IMPORTANT!!!!!!
# This is the version WITH the fixed VCMAX2 computation.
# 2011-09-01
#########################################################
of_text = <<END
    // Response Variables
    ...
    double LDMAX2 = 17.780001 + 0.4845 * CSPD2 + 1.625 * AR2 + 0.0267
        ...
        * pow(SEATW2, 2);
    double VCMAX2 = 200.4 - 0.3799 * CSPD2 + 0.8236 * AR2 + 0.2168 
        * SWEEP2 + 1.74 * DPROP2 + 5.589 * WINGLD2 - 0.1683 * AF2 
        - 3.522 * SEATW2 + 1.559 * ELODT2 + 0.2442 * TAPER2 - 0.0215 
        * CSPD2 * AR2 + 0.0054 * CSPD2 * SWEEP2 - 0.0465 * CSPD2 * DPROP2 
        - 0.0144 * CSPD2 * WINGLD2 - 0.0069 * CSPD2 * AF2 + 0.0578 
        * CSPD2 * SEATW2 + 0.0557 * CSPD2 * ELODT2 + 0.0221 * CSPD2 
        * TAPER2 + 0.0175 * AR2 * SWEEP2 - 0.0942 * AR2 * DPROP2 + 0.1402 
        * AR2 * WINGLD2 - 0.0079 * AR2 * AF2 + 0.0147 * AR2 * SEATW2 
        - 0.071 * AR2 * ELODT2 - 0.0104 * AR2 * TAPER2 - 0.0067 * SWEEP2 
        * DPROP2 - 0.0306 * SWEEP2 * WINGLD2 - 0.0078 * SWEEP2 * AF2 
        - 0.0534 * SWEEP2 * SEATW2 - 0.0224 * SWEEP2 * ELODT2 + 0.0026 
        * SWEEP2 * TAPER2 - 0.2771 * DPROP2 * WINGLD2 - 0.3476 * DPROP2 
        * AF2 + 0.2093 * DPROP2 * SEATW2 - 0.0864 * DPROP2 * ELODT2 
        - 0.0051 * DPROP2 * TAPER2 - 0.0948 * WINGLD2 * AF2 - 0.7081 
        * WINGLD2 * SEATW2 - 0.1219 * WINGLD2 * ELODT2 - 0.016 * WINGLD2 
        * TAPER2 + 0.0555 * AF2 * SEATW2 - 0.0342 * AF2 * ELODT2 - 0.0049 
        * AF2 * TAPER2 - 0.3658 * SEATW2 * ELODT2 - 0.046 * SEATW2 * TAPER2 
        - 0.0504 * ELODT2 * TAPER2 - 0.1452 * pow(CSPD2, 2) - 0.2937 
        * pow(AR2, 2) + 0.0303 * pow(SWEEP2, 2) + 0.1378 * pow(DPROP2, 2) 
        - 0.6522 * pow(WINGLD2, 2) + 0.1378 * pow(AF2, 2) - 0.6102 
        * pow(SEATW2, 2) - 0.3722 * pow(ELODT2, 2) + 0.0303 * pow(TAPER2, 2);
...
columns="constant|CSPD|AR|SWEEP|DPROP|WINGLD|AF|SEATW|ELODT|TAPER|CSPD*AR|CSPD*SWEEP|CSPD*DPROP|CSPD*WINGLD|CSPD*AF|CSPD*SEATW|CSPD*ELODT|CSPD*TAPER|AR*SWEEP|AR*DPROP|AR*WINGLD|AR*AF|AR*SEATW|AR*ELODT|AR*TAPER|SWEEP*DPROP|SWEEP*WINGLD|SWEEP*AF|SWEEP*SEATW|SWEEP*ELODT|SWEEP*TAPER|DPROP*WINGLD|DPROP*AF|DPROP*SEATW|DPROP*ELODT|DPROP*TAPER|WINGLD*AF|WINGLD*SEATW|WINGLD*ELODT|WINGLD*TAPER|AF*SEATW|AF*ELODT|AF*TAPER|SEATW*ELODT|SEATW*TAPER|ELODT*TAPER|pow(CSPD,2)|pow(AR,2)|pow(SWEEP,2)|pow(DPROP,2)|pow(WINGLD,2)|pow(AF,2)|pow(SEATW,2)|pow(ELODT,2)|pow(TAPER,2)"
@cols = columns.split("|")

# fortunately the subset of C used to describe this stuff is pretty 
# straightforward, so even though I don't know how to write a parser,
# I can figure this stuff out

# split an equation into chunks
def chunkify(equation)
  chunks = []
  accumulator = ""
  # equation.each_char do |char| ruby 1.9
  equation.each do |char| #ruby 1.8
    if(["=",'+','-'].include? char)
      chunks << accumulator
      accumulator = ""
    end
...
# split into coefficient / variable pairs
def chunks_to_hash(chunks)
  coefficients = Hash.new(0)
  chunks.each do |chunk|
    # break at the * sign
    splits = chunk.partition("*")
    if(splits[2].size ==0)
      splits[2] = "constant"
    end
    # get rid of the number, e.g. AR2 becomes AR
    splits[2].gsub!(/([A-Z]+)[0-9]/ , '\1')
    # key = decision variable, value = coefficient
    coefficients[splits[2]] = splits[0]
  end
  coefficients
end

# flatten the hash into an array
def flatten(coefficients)
  arry = Array.new
  @cols.each do |col|
    arry << coefficients[col]
  end
  arry
end

# First, split into individual equations
equations = of_text.split(/^ *double/)
equations.reject!{|eq| !(/=/ =~ eq)}

# collect rows for the table
coeff_matrix = equations.collect do |equation|
  chunks = chunkify(equation)

  # process each chunk into coefficient and variable
  objective = chunks.shift
  coefficients = chunks_to_hash(chunks)

  # return a table row
  [objective, *flatten(coefficients)]
end


# print out the whole matrix
puts "objective\t#{@cols.join("\t")}"
coeff_matrix.each do |row|
  puts row.join("\t")
end
